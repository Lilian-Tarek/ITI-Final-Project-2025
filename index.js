...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (offcanvas) {
    ...
    if (cart.length === 0) {
      // Show empty cart message
      offcanvas.innerHTML = `<p>Cart is empty</p>`;
      return;
    }
    ...
    cart.forEach((item, index) => {
      ...
      offdiv.innerHTML = `
        <div class="d-flex my-3 align-items-center gap-3"><img src="${item.image}" alt="${item.name}" width="60"class="rounded w-25"/>
        <p class="w-25">${item.name}</p>
        <p class="w-25">${item.price}</p>
        <p class="w-25"><button class="btn btn-brown delete-item" data-index="${index}">🗑️</button></p>
        </div>
      `;
    ...
    footer.innerHTML = `
  <button id="checkout" class="btn btn-dark w-100">
<a href="./cart.html"class="text-decoration-none text-white">  Go to the Cart</a>
  </button>
`;
    offcanvas.appendChild(footer);
